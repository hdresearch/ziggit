...
var global_config_overrides: ?std.ArrayList(ConfigOverride) = null;
...
pub fn zigzitMain(allocator: std.mem.Allocator) !void {
    ...
    // Commands that forward to real git for full compatibility
        ...
        try cmdSwitch(allocator, &args_iter, &platform_impl);
    } else if (std.mem.eql(u8, command, "restore")) {
        try cmdRestore(allocator, &args_iter, &platform_impl);
    ...
    for (index.entries.items) |entry| {
        ...
        if (!file_exists) {
            ...
            const working_modified = blk: {
                ...
                const index_hash = std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)}) catch break :blk false;
...
fn showWorkingTreeDiff(index: *const index_mod.Index, cwd: []const u8, platform_impl: *const platform_mod.Platform, allocator: std.mem.Allocator, quiet: bool) !bool {
    ...
    for (index.entries.items) |entry| {
        ...
        if (platform_impl.fs.exists(full_path) catch false) {
            ...
            // Compare with index hash
            const index_hash = try std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)});
            defer allocator.free(index_hash);
...
fn getTreeEntryHashByPath(git_path: []const u8, tree_hash: []const u8, file_path: []const u8, allocator: std.mem.Allocator) ![]u8 {
    ...
    while (pos < tree_content.len) {
        ...
        if (std.mem.eql(u8, name, first_component)) {
            ...
            _ = std.fmt.bufPrint(&hex_buf, "{s}", .{std.fmt.fmtSliceHexLower(entry_hash_bytes[0..20])}) catch return error.InvalidHash;
...
fn getIndexedFileContent(entry: index_mod.IndexEntry, allocator: std.mem.Allocator) ![]u8 {
    ...
    _ = std.fmt.bufPrint(hash_str, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)}) catch |err| {
...
fn cmdCheckout(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    if (std.mem.eql(u8, first_arg, "-b")) {
        ...
        refs.createBranch(git_path, branch_name, start_point, platform_impl, allocator) catch |err| {
            // For -B on empty repo (no commits yet), just set HEAD
            switch (err) {
...
fn checkoutTreeRecursive(git_path: []const u8, tree_data: []const u8, repo_root: []const u8, current_path: []const u8, allocator: std.mem.Allocator, platform_impl: *const platform_mod.Platform) !void {
    ...
    while (i < tree_data.len) {
        ...
        _ = std.fmt.bufPrint(hash_hex, "{s}", .{std.fmt.fmtSliceHexLower(hash_bytes)}) catch break;
...
fn parseTreeIntoMap(tree_data: []const u8, file_map: *std.StringHashMap([]const u8), allocator: std.mem.Allocator) !void {
    ...
    while (i < tree_data.len) {
        ...
        const hash_bytes = tree_data[i..i + 20];
        const hash_hex = try allocator.alloc(u8, 40);
        _ = std.fmt.bufPrint(hash_hex, "{s}", .{std.fmt.fmtSliceHexLower(hash_bytes)}) catch {
...
fn cmdConfig(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    if (do_get_all) {
        ...
        for (sources.items) |source| {
            const content = platform_impl.fs.readFile(allocator, source.path) catch continue;
            defer allocator.free(content);
            var all_vals = std.ArrayList([]const u8).init(allocator);
            defer {
...
fn parseConfigGetAll(content: []const u8, key: []const u8, results: *std.ArrayList([]const u8), allocator: std.mem.Allocator) !void {
    var lines = std.mem.splitSequence(u8, content, "\n");
    var current_section: ?[]u8 = null;
    defer if (current_section) |s| allocator.free(s);
...
fn configSetValue(cfg_path: []const u8, key: []const u8, value: []const u8, do_add: bool, allocator: std.mem.Allocator, platform_impl: *const platform_mod.Platform) !void {
    ...
    if (!found) {
        if (section_found) {
            ...
            var insert_result = std.ArrayList(u8).init(allocator);
    ...
    while (line_iter.next()) |raw_line| {
        ...
        if (trimmed[0] == '[') {
            ...
            if (std.mem.indexOf(u8, after_bracket, "=")) |inline_eq_pos| {
                ...
                var inline_value_buf = std.ArrayList(u8).init(allocator);
...
fn appendConfigValuePart(buf: *std.ArrayList(u8), raw: []const u8) !void {
    ...
    while (i < tree_obj.data.len) {
        ...
        if (std.mem.eql(u8, name, name_to_find)) {
            if (remaining) |rest| {
                ...
                const sub_tree_hash = try std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(hash_bytes)});
...
fn cmdLsFiles(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
        ...
        for (index.entries.items) |entry| {
            ...
            if (modified_flag) {
                ...
                if (!file_exists2) {
                    ...
                    const index_hash2 = std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)}) catch continue;
    ...
    if (cached) {
        ...
        for (index.entries.items) |entry| {
            ...
            if (stage) {
                const hash_str = try std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)});
                defer allocator.free(hash_str);
...
fn getObjectSize(allocator: std.mem.Allocator, git_path: []const u8, sha1: *const [20]u8, platform_impl: anytype) !u64 {
    ...
    const hex = try std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(sha1)});
...
fn revListCollectTree(allocator: std.mem.Allocator, git_path: []const u8, tree_hash: []const u8, prefix: []const u8, no_names: bool, emitted: *std.StringHashMap(void), deferred: *std.ArrayList([]const u8), platform_impl: *const platform_mod.Platform) !void {
    if (emitted.contains(tree_hash)) return;
    try emitted.put(try allocator.dupe(u8, tree_hash), {});
...
fn collectTreeEntries(git_path: []const u8, tree_hash: []const u8, prefix: []const u8, platform_impl: *const platform_mod.Platform, allocator: std.mem.Allocator, entries: *std.ArrayList(index_mod.IndexEntry)) !void {
...
fn cmdHashObject(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    if (stdin_paths) {
        ...
        const stdin_data = std.io.getStdIn().readToEndAlloc(allocator, 10 * 1024 * 1024) catch {
            std.process.exit(128);
            unreachable;
        };
        defer allocator.free(stdin_data);
        ...
        const data = std.io.getStdIn().readToEndAlloc(allocator, 100 * 1024 * 1024) catch {
            std.process.exit(128);
            unreachable;
        };
...
fn cmdCommitTree(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    if (read_stdin and message == null) {
        final_message = std.io.getStdIn().readToEndAlloc(allocator, 10 * 1024 * 1024) catch {
...
fn parseTreeEntries(tree_data: []const u8, allocator: std.mem.Allocator) !std.ArrayList(LsTreeEntry) {
...
fn cmdDiffFiles(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    for (idx.entries.items) |entry| {
        ...
        if (!file_exists) {
            ...
            _ = std.fmt.bufPrint(&hash_buf, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)}) catch unreachable;
            if (name_only) {
...
fn collectLooseRefs(allocator: std.mem.Allocator, git_dir: []const u8, prefix: []const u8, ref_list: *std.ArrayList(RefEntry), platform_impl: anytype) !void {
...
fn doNativeRepack(allocator: std.mem.Allocator, git_dir: []const u8, platform_impl: anytype, quiet: bool) !void {
    ...
    var pack_entries = std.ArrayList(PackIdxEntry).init(allocator);
...
fn parsePatchSet(allocator: std.mem.Allocator, data: []const u8, p_value: u32, corrupt_line: *usize) !std.ArrayList(Patch) {
...
fn buildTreeMap(tree_hash: []const u8, prefix: []const u8, git_path: []const u8, platform_impl: *const platform_mod.Platform, allocator: std.mem.Allocator, map: *std.StringHashMap([20]u8)) !void {
    ...
    while (i < tree_obj.data.len) {
        ...
        if (std.mem.eql(u8, mode_str, "40000") or std.mem.eql(u8, mode_str, "040000")) {
            // Directory - recurse
            const sub_hash = try std.fmt.allocPrint(allocator, "{s}", .{std.fmt.fmtSliceHexLower(hash_bytes[0..20])});
...
fn cmdSwitch(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    if (@import("builtin").target.os.tag == .freestanding) {
    ...
    if (detach) {
        ...
        const hash = resolveRevision(git_path, t, platform_impl, allocator) catch {
        ...
        defer allocator.free(hash);
        try checkoutCommitTree(git_path, hash, allocator, platform_impl);
        const head_path = try std.fmt.allocPrint(allocator, "{s}/HEAD", .{git_path});
        defer allocator.free(head_path);
        const head_content = try std.fmt.allocPrint(allocator, "{s}\n", .{hash});
        defer allocator.free(head_content);
...
fn cmdRestore(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
...
fn cmdWorktree(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    const subcmd = args.next() orelse {
...
fn cmdStripspace(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    // Strip trailing whitespace, collapse blank lines, strip comments
    var result = std.ArrayList(u8).init(allocator);
    defer result.deinit();
    ...
    while (lines.next()) |line| {
        ...
        // Add at most one blank line between content
...
fn cmdCheckoutIndex(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    for (idx.entries.items) |*entry| {
        ...
        var hash_buf: [40]u8 = undefined;
        _ = std.fmt.bufPrint(&hash_buf, "{s}", .{std.fmt.fmtSliceHexLower(&entry.sha1)}) catch continue;
