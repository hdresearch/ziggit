...
/// Compress a slice to a newly allocated slice using the C zlib library.
pub fn compressSlice(allocator: std.mem.Allocator, input: []const u8) ![]u8 {
    ...
    const result = try allocator.realloc(dest, @intCast(dest_len));
    return result;
}

/// Decompress a slice to a newly allocated slice.
...
/// Decompress a slice and report how many compressed bytes were consumed.
pub fn decompressSliceWithConsumed(allocator: std.mem.Allocator, input: []const u8) !struct { data: []u8, consumed: usize } {
    ...
    const consumed = input.len - @as(usize, @intCast(stream.avail_in));
    return .{ .data = try result.toOwnedSlice(), .consumed = consumed };
}
