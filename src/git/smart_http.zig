...
fn httpGetWithClientOpts(allocator: std.mem.Allocator, existing_client: ?*std.http.Client, url: []const u8, v2: bool) ![]u8 {
    ...
    var resp = req.receiveHead(&redirect_buf) catch return error.HttpError;
...
fn httpPostWithClientOpts(allocator: std.mem.Allocator, existing_client: ?*std.http.Client, url: []const u8, body: []const u8, content_type: []const u8, v2: bool) ![]u8 {
    ...
    // Send body using the new API
    req.transfer_encoding = .{ .content_length = body.len };
    var bw = req.sendBodyUnflushed(&.{}) catch return error.HttpError;
    bw.writer.writeAll(body) catch return error.HttpError;
    bw.end() catch return error.HttpError;
    ...
    const result = blk: {
        ...
        break :blk rdr.allocRemaining(allocator, std.io.Limit.limited(max_response_size)) catch return error.HttpError;
    };
