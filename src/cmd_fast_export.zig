...
pub fn cmdFastExport(allocator: std.mem.Allocator, args: *platform_mod.ArgIterator, platform_impl: *const platform_mod.Platform) !void {
    ...
    {
        var cm_ts = std.StringHashMap(std.ArrayListUnmanaged([]const u8)).init(allocator);
        defer {
        ...
        for (all_commits.items) |ch| {
            if (!cm_ts.contains(ch)) {
                try cm_ts.put(ch, std.ArrayListUnmanaged([]const u8){});
                try id_ts.put(ch, 0);
            }
        }
        ...
        var qt: std.ArrayListUnmanaged([]const u8) = .{};
        ...
        var st: std.ArrayListUnmanaged([]const u8) = .{};
