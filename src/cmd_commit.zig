...
fn cleanupWhitespace(allocator: std.mem.Allocator, msg: []const u8) ![]const u8 {
    // Strip trailing whitespace from each line and collapse trailing empty lines
    ...
    while (lines.next()) |line| {
        ...
        try result.appendSlice(line[0..end]);
    }
    // Remove trailing empty lines, keep one trailing newline
    var rlen = result.items.len;
    while (rlen > 0 and result.items[rlen - 1] == '\n') rlen -= 1;
    result.shrinkRetainingCapacity(rlen);
    if (rlen > 0) try result.append('\n');
    return result.items;
}
...
fn cleanupStrip(allocator: std.mem.Allocator, msg: []const u8) ![]const u8 {
    ...
    // Remove trailing empty lines, keep one trailing newline
    var rlen = result.items.len;
    while (rlen > 1 and result.items[rlen - 1] == '\n' and result.items[rlen - 2] == '\n') rlen -= 1;
    result.shrinkRetainingCapacity(rlen);
    return result.items;
}

fn cleanupScissors(allocator: std.mem.Allocator, msg: []const u8) ![]const u8 {
    // Strip everything below scissors line "-- >8 --", then apply strip cleanup
    const scissors = "-- >8 --";
    var end_pos = msg.len;
    var lines = std.mem.splitScalar(u8, msg, '\n');
    var pos: usize = 0;
    while (lines.next()) |line| {
        if (std.mem.indexOf(u8, line, scissors) != null) {
            end_pos = pos;
            break;
        }
        pos += line.len + 1;
    }
    return cleanupStrip(allocator, msg[0..@min(end_pos, msg.len)]);
}
