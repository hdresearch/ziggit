...
fn testPlainInit(tf: *TestFramework) !void {
    ...
    // Test ziggit init
    const ziggit_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init" }, "test-plain");
...
fn testBareInit(tf: *TestFramework) !void {
    ...
    const ziggit_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init", "--bare", "test-bare.git" }, null);
    defer tf.*.allocator.free(ziggit_result.stdout);
    defer tf.*.allocator.free(ziggit_result.stderr);
...
fn testInitInExistingDir(tf: *TestFramework) !void {
    ...
    // Test ziggit init
    const ziggit_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init" }, "test-existing");
    defer tf.*.allocator.free(ziggit_result.stdout);
    defer tf.*.allocator.free(ziggit_result.stderr);
...
fn testReinitialize(tf: *TestFramework) !void {
    ...
    // First init
    const first_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init" }, "test-reinit");
    defer tf.*.allocator.free(first_result.stdout);
    defer tf.*.allocator.free(first_result.stderr);
    
    // Second init (reinitialize)
    const second_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init" }, "test-reinit");
...
fn testInitTemplate(tf: *TestFramework) !void {
    ...
    const ziggit_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init", "--template=/dev/null", "test-template" }, null);
    defer tf.*.allocator.free(ziggit_result.stdout);
    defer tf.*.allocator.free(ziggit_result.stderr);
...
fn testInitWithDir(tf: *TestFramework) !void {
    ...
    const ziggit_result = try tf.runCommand(&[_][]const u8{ "../zig-out/bin/ziggit", "init", "test-newdir" }, null);
    defer tf.*.allocator.free(ziggit_result.stdout);
    defer tf.*.allocator.free(ziggit_result.stderr);
