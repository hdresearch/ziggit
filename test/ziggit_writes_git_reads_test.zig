...
// ============================================================
// Additional cross-validation tests: ziggit writes, git reads
// ============================================================

test "ziggit commit -> git fsck validates entire repo" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Multiple commits with different file types
    try writeFileHelper(allocator, tmp, "README.md", "# Hello\nWorld\n");
    try repo.add("README.md");
    _ = try repo.commit("first", "Alice", "alice@test.com");

    try writeFileHelper(allocator, tmp, "src/main.zig", "const std = @import(\"std\");\n");
    try repo.add("src/main.zig");
    _ = try repo.commit("add source", "Bob", "bob@test.com");
    ...
    const fsck = try execGit(allocator, tmp, &.{"fsck"});
    defer allocator.free(fsck);
    // fsck output on success is empty or just warnings (dangling objects etc.)
    // It should NOT contain "error" or "invalid"
    try std.testing.expect(std.mem.indexOf(u8, fsck, "error in ") == null);
    try std.testing.expect(std.mem.indexOf(u8, fsck, "invalid ") == null);
}

test "ziggit annotated tag -> git cat-file shows tag object with tagger" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "file.txt", "content");
    try repo.add("file.txt");
    const commit_hash = try repo.commit("initial", "T", "t@t");
    try repo.createTag("v2.0.0", "Release 2.0.0");

    // git cat-file -t should say "tag"
    const obj_type = try execGit(allocator, tmp, &.{ "cat-file", "-t", "v2.0.0" });
    defer allocator.free(obj_type);
    try std.testing.expectEqualStrings("tag", trimRight(obj_type));

    // git cat-file -p should show tag fields
    const tag_content = try execGit(allocator, tmp, &.{ "cat-file", "-p", "v2.0.0" });
    defer allocator.free(tag_content);
    try std.testing.expect(std.mem.indexOf(u8, tag_content, "object ") != null);
    try std.testing.expect(std.mem.indexOf(u8, tag_content, "tag v2.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, tag_content, "Release 2.0.0") != null);

    // The tag object target should point to our commit
    const tag_target = try execGit(allocator, tmp, &.{ "rev-parse", "v2.0.0^{commit}" });
    defer allocator.free(tag_target);
    try std.testing.expectEqualStrings(&commit_hash, trimRight(tag_target));
}

test "ziggit 100+ files commit -> git ls-tree lists all files" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    _ = try repo.commit("add 120 files", "T", "t@t");

    // git ls-tree should list all files
    const ls_tree = try execGit(allocator, tmp, &.{ "ls-tree", "--name-only", "HEAD" });
    defer allocator.free(ls_tree);

    var count: usize = 0;
    var lines = std.mem.splitScalar(u8, trimRight(ls_tree), '\n');
    while (lines.next()) |line| {
        if (line.len > 0) count += 1;
    }
    try std.testing.expectEqual(file_count, count);
}

test "ziggit deeply nested 10 levels -> git cat-file validates tree chain" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Create deeply nested file: a/b/c/d/e/f/g/h/i/j/deep.txt
    const deep_path = "a/b/c/d/e/f/g/h/i/j/deep.txt";
    try writeFileHelper(allocator, tmp, deep_path, "deep content");
    try repo.add(deep_path);
    _ = try repo.commit("deep nesting", "T", "t@t");

    // git should be able to show the deep file
    const content = try execGit(allocator, tmp, &.{ "show", "HEAD:a/b/c/d/e/f/g/h/i/j/deep.txt" });
    defer allocator.free(content);
    try std.testing.expectEqualStrings("deep content", trimRight(content));
...
test "ziggit binary file with null bytes -> git show preserves content" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Write binary file with all byte values 0-255
    ...
    try repo.add("binary.dat");
    _ = try repo.commit("binary file", "T", "t@t");

    // git cat-file -s should report 256 bytes
    const size = try execGit(allocator, tmp, &.{ "cat-file", "-s", "HEAD:binary.dat" });
    defer allocator.free(size);
    try std.testing.expectEqualStrings("256", trimRight(size));
}

test "ziggit files with spaces -> git show retrieves correctly" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "my file.txt", "content with spaces");
    try repo.add("my file.txt");
    try writeFileHelper(allocator, tmp, "dir with space/nested file.txt", "nested content");
    try repo.add("dir with space/nested file.txt");
    _ = try repo.commit("spaces in names", "T", "t@t");

    // git should see both files
    const ls = try execGit(allocator, tmp, &.{ "ls-tree", "-r", "--name-only", "HEAD" });
    defer allocator.free(ls);
    try std.testing.expect(std.mem.indexOf(u8, ls, "my file.txt") != null);
    try std.testing.expect(std.mem.indexOf(u8, ls, "dir with space/nested file.txt") != null);
}

test "ziggit describe matches git describe at exact tag" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "f.txt", "v1");
    try repo.add("f.txt");
    _ = try repo.commit("release", "T", "t@t");
    try repo.createTag("v3.0.0", null);

    // Exact tag: both should say "v3.0.0"
    const ziggit_desc = try repo.describeTags(allocator);
    defer allocator.free(ziggit_desc);
    const git_desc = try execGit(allocator, tmp, &.{ "describe", "--tags" });
    defer allocator.free(git_desc);
    try std.testing.expectEqualStrings(trimRight(ziggit_desc), trimRight(git_desc));

    // Add commits after tag and verify ziggit still references the tag
    try writeFileHelper(allocator, tmp, "f2.txt", "v2");
    try repo.add("f2.txt");
    _ = try repo.commit("post-release", "T", "t@t");

    var repo2 = try ziggit.Repository.open(allocator, tmp);
    defer repo2.close();
    const ziggit_desc2 = try repo2.describeTags(allocator);
    defer allocator.free(ziggit_desc2);
    // ziggit describe should at least contain the tag name
    try std.testing.expect(std.mem.indexOf(u8, ziggit_desc2, "v3.0.0") != null);
}

test "ziggit commit author/committer format matches git expectation" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "f.txt", "hello");
    try repo.add("f.txt");
    _ = try repo.commit("test msg", "Jane Doe", "jane@example.com");

    // git cat-file should show proper author/committer lines
    const cat = try execGit(allocator, tmp, &.{ "cat-file", "-p", "HEAD" });
    defer allocator.free(cat);
    try std.testing.expect(std.mem.indexOf(u8, cat, "author Jane Doe <jane@example.com>") != null);
    try std.testing.expect(std.mem.indexOf(u8, cat, "committer Jane Doe <jane@example.com>") != null);
    try std.testing.expect(std.mem.indexOf(u8, cat, "\n\ntest msg") != null);
}

test "ziggit multiple tags -> git tag -l lists all sorted" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const tags = [_][]const u8{ "v0.1.0", "v0.2.0", "v0.3.0", "v1.0.0", "v2.0.0-beta.1" };
    for (tags) |tag| {
        var buf: [32]u8 = undefined;
        const name = std.fmt.bufPrint(&buf, "{s}.txt", .{tag}) catch unreachable;
        try writeFileHelper(allocator, tmp, name, tag);
        try repo.add(name);
        _ = try repo.commit(tag, "T", "t@t");
        try repo.createTag(tag, null);
    }

    // git tag -l should list all
    const git_tags = try execGit(allocator, tmp, &.{ "tag", "-l" });
    defer allocator.free(git_tags);
    for (tags) |tag| {
        try std.testing.expect(std.mem.indexOf(u8, git_tags, tag) != null);
    }
}

test "ziggit multi-file commit -> git rev-parse HEAD^{tree} valid and ls-tree correct" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "a.txt", "aaa");
    try writeFileHelper(allocator, tmp, "b.txt", "bbb");
    try writeFileHelper(allocator, tmp, "sub/c.txt", "ccc");
    try repo.add("a.txt");
    try repo.add("b.txt");
    try repo.add("sub/c.txt");
    _ = try repo.commit("multi-file", "T", "t@t");

    // git should be able to resolve tree hash
    const tree_hash = try execGit(allocator, tmp, &.{ "rev-parse", "HEAD^{tree}" });
    defer allocator.free(tree_hash);
    // Should be a valid 40-char hex hash
    const trimmed = trimRight(tree_hash);
    try std.testing.expectEqual(@as(usize, 40), trimmed.len);

    // Verify tree contents match our files
    const ls = try execGit(allocator, tmp, &.{ "ls-tree", "-r", "--name-only", "HEAD" });
    defer allocator.free(ls);
    try std.testing.expect(std.mem.indexOf(u8, ls, "a.txt") != null);
    try std.testing.expect(std.mem.indexOf(u8, ls, "b.txt") != null);
    try std.testing.expect(std.mem.indexOf(u8, ls, "sub/c.txt") != null);
}

test "ziggit bun publish workflow -> git validates every step" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "index.js", "module.exports = {};\n");
    try repo.add("package.json");
    try repo.add("index.js");
    _ = try repo.commit("initial release", "Publisher", "pub@test.com");
    try repo.createTag("v1.0.0", null);

    // Verify: git status clean
    const status1 = try execGit(allocator, tmp, &.{ "status", "--porcelain" });
    defer allocator.free(status1);
    try std.testing.expectEqualStrings("", trimRight(status1));

    // Verify: git describe matches tag
    const desc1 = try execGit(allocator, tmp, &.{ "describe", "--tags" });
    defer allocator.free(desc1);
    try std.testing.expectEqualStrings("v1.0.0", trimRight(desc1));

    // Verify: git log shows 1 commit
    const log_count = try execGit(allocator, tmp, &.{ "rev-list", "--count", "HEAD" });
    defer allocator.free(log_count);
    try std.testing.expectEqualStrings("1", trimRight(log_count));
    ...
    // Verify: git show HEAD:package.json has version
    const pkg = try execGit(allocator, tmp, &.{ "show", "HEAD:package.json" });
    defer allocator.free(pkg);
    try std.testing.expect(std.mem.indexOf(u8, pkg, "1.0.0") != null);

    // Verify: git show HEAD:index.js has content
    const idx = try execGit(allocator, tmp, &.{ "show", "HEAD:index.js" });
    defer allocator.free(idx);
    try std.testing.expect(std.mem.indexOf(u8, idx, "module.exports") != null);
}

test "ziggit add + commit preserves file content byte-for-byte" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "test.txt", test_content);
    try repo.add("test.txt");
    _ = try repo.commit("content check", "T", "t@t");

    // git show should return exact content
    const content = try execGit(allocator, tmp, &.{ "show", "HEAD:test.txt" });
    defer allocator.free(content);
    try std.testing.expectEqualStrings(test_content, content);
}

test "ziggit 5 sequential commits -> git log --oneline shows all" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    for (0..5) |i| {
        var name_buf: [32]u8 = undefined;
        const name = std.fmt.bufPrint(&name_buf, "commit{d}.txt", .{i}) catch unreachable;
        var msg_buf: [32]u8 = undefined;
        const msg = std.fmt.bufPrint(&msg_buf, "commit number {d}", .{i}) catch unreachable;
        try writeFileHelper(allocator, tmp, name, msg);
        try repo.add(name);
        hashes[i] = try repo.commit(msg, "T", "t@t");
    }

    // Verify each commit via git rev-parse HEAD~N
    for (0..5) |i| {
        var ref_buf: [16]u8 = undefined;
        const ref = std.fmt.bufPrint(&ref_buf, "HEAD~{d}", .{4 - i}) catch unreachable;
        const parsed = try execGit(allocator, tmp, &.{ "rev-parse", ref });
        defer allocator.free(parsed);
        try std.testing.expectEqualStrings(&hashes[i], trimRight(parsed));
    }
}

test "ziggit repo -> git verify-pack after repack succeeds" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Create several commits to give repack something to work with
    for (0..10) |i| {
        var name_buf: [32]u8 = undefined;
        const name = std.fmt.bufPrint(&name_buf, "file{d}.txt", .{i}) catch unreachable;
        var content_buf: [64]u8 = undefined;
        const content = std.fmt.bufPrint(&content_buf, "content for file {d} with some padding", .{i}) catch unreachable;
        try writeFileHelper(allocator, tmp, name, content);
        try repo.add(name);
        _ = try repo.commit("commit", "T", "t@t");
    }

    // git gc to repack everything
    _ = try execGit(allocator, tmp, &.{ "gc", "--aggressive" });

    // git verify-pack on all pack files should succeed
    const pack_dir = try std.fmt.allocPrint(allocator, "{s}/.git/objects/pack", .{tmp});
    defer allocator.free(pack_dir);
    ...
    while (try iter.next()) |entry| {
        if (std.mem.endsWith(u8, entry.name, ".pack")) {
            const pack_path = try std.fmt.allocPrint(allocator, "{s}/{s}", .{ pack_dir, entry.name });
            defer allocator.free(pack_path);
            const verify = try execGit(allocator, tmp, &.{ "verify-pack", "-v", pack_path });
            defer allocator.free(verify);
            // Should not contain "error"
            try std.testing.expect(std.mem.indexOf(u8, verify, "error") == null);
        }
    }
}

// === NEW: Round-trip and cross-validation edge cases ===

test "ziggit commit -> git clone -> verify blob content matches" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const content = "exact content: hello, world!\nline2\n";
    try writeFileHelper(allocator, tmp, "data.txt", content);
    try repo.add("data.txt");
    _ = try repo.commit("exact check", "T", "t@t");
    repo.close();

    // git clone and verify content
    ...
    const blob = try execGit(allocator, clone_dir, &.{ "show", "HEAD:data.txt" });
    defer allocator.free(blob);
    try std.testing.expectEqualStrings(content, blob);
}

test "ziggit two commits -> git diff HEAD~1 HEAD shows exactly changed file" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "stable.txt", "unchanged");
    try writeFileHelper(allocator, tmp, "changing.txt", "version 1");
    try repo.add("stable.txt");
    try repo.add("changing.txt");
    _ = try repo.commit("initial", "T", "t@t");

    try writeFileHelper(allocator, tmp, "changing.txt", "version 2");
    try repo.add("changing.txt");
    _ = try repo.commit("update", "T", "t@t");

    // git diff should show only changing.txt
    const diff_stat = try execGit(allocator, tmp, &.{ "diff", "--name-only", "HEAD~1", "HEAD" });
    defer allocator.free(diff_stat);
    const trimmed = trimRight(diff_stat);
    try std.testing.expectEqualStrings("changing.txt", trimmed);
}

test "ziggit commit -> git cat-file shows correct object sizes" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const data = "abcdefghij"; // exactly 10 bytes
    try writeFileHelper(allocator, tmp, "sized.txt", data);
    try repo.add("sized.txt");
    _ = try repo.commit("sized", "T", "t@t");

    // Get blob hash from tree
    const tree = try execGit(allocator, tmp, &.{ "ls-tree", "HEAD", "sized.txt" });
    defer allocator.free(tree);
    // Parse blob hash (format: "100644 blob <hash>\t<name>")
    const trimmed_tree = trimRight(tree);
    var parts = std.mem.splitScalar(u8, trimmed_tree, ' ');
    _ = parts.next(); // mode
    _ = parts.next(); // type
    const rest = parts.next() orelse return error.ParseError;
    var tab_parts = std.mem.splitScalar(u8, rest, '\t');
    const blob_hash = tab_parts.next() orelse return error.ParseError;

    const size_out = try execGit(allocator, tmp, &.{ "cat-file", "-s", blob_hash });
    defer allocator.free(size_out);
    const size_str = trimRight(size_out);
    try std.testing.expectEqualStrings("10", size_str);
}

test "ziggit multiple tags -> git for-each-ref lists all with correct type" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "f.txt", "tagged");
    try repo.add("f.txt");
    _ = try repo.commit("tagged", "T", "t@t");

    try repo.createTag("v1.0.0", null);
    try repo.createTag("v2.0.0", null);
    try repo.createTag("v3.0.0", null);

    const refs = try execGit(allocator, tmp, &.{ "for-each-ref", "--format=%(refname:short)", "refs/tags/" });
    defer allocator.free(refs);
    const trimmed = trimRight(refs);

    // Should contain all 3 tags
    try std.testing.expect(std.mem.indexOf(u8, trimmed, "v1.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, trimmed, "v2.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, trimmed, "v3.0.0") != null);
...
test "ziggit overwrite file -> git log -p shows correct diffs" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "version.txt", "1.0.0\n");
    try repo.add("version.txt");
    _ = try repo.commit("v1", "T", "t@t");

    try writeFileHelper(allocator, tmp, "version.txt", "2.0.0\n");
    try repo.add("version.txt");
    _ = try repo.commit("v2", "T", "t@t");

    // git log -p should show both versions
    const log = try execGit(allocator, tmp, &.{ "log", "-p", "--format=%s" });
    defer allocator.free(log);
    try std.testing.expect(std.mem.indexOf(u8, log, "1.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, log, "2.0.0") != null);
}

test "ziggit commit -> git rev-list --all --objects lists all objects" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const objects = try execGit(allocator, tmp, &.{ "rev-list", "--all", "--objects" });
    defer allocator.free(objects);

    // Count objects: at least 1 commit + 1 tree + 2 blobs = 4
    var count: usize = 0;
    var lines = std.mem.splitScalar(u8, trimRight(objects), '\n');
    while (lines.next()) |line| {
        if (line.len > 0) count += 1;
    }
...
test "ziggit init creates valid HEAD pointing to master" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Read HEAD directly
    const head_path = try std.fmt.allocPrint(allocator, "{s}/.git/HEAD", .{tmp});
    defer allocator.free(head_path);
    const head_content = try std.fs.cwd().readFileAlloc(allocator, head_path, 4096);
    defer allocator.free(head_content);

    // Should be a symbolic ref to a branch
    try std.testing.expect(std.mem.startsWith(u8, head_content, "ref: refs/heads/"));

    // git symbolic-ref should also work
    const symbolic = try execGit(allocator, tmp, &.{ "symbolic-ref", "HEAD" });
    defer allocator.free(symbolic);
    try std.testing.expect(std.mem.startsWith(u8, trimRight(symbolic), "refs/heads/"));
}

test "ziggit commit -> git shortlog lists author" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "f.txt", "data");
    try repo.add("f.txt");
    _ = try repo.commit("shortlog test", "TestAuthor", "author@test.com");

    const shortlog = try execGit(allocator, tmp, &.{ "shortlog", "-s", "-n", "HEAD" });
    defer allocator.free(shortlog);
    try std.testing.expect(std.mem.indexOf(u8, shortlog, "TestAuthor") != null);
}

test "ziggit binary with all 256 byte values -> git cat-file preserves exactly" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const fp = try std.fmt.allocPrint(allocator, "{s}/binary.bin", .{tmp});
    defer allocator.free(fp);
    const f = try std.fs.createFileAbsolute(fp, .{ .truncate = true });
    try f.writeAll(&binary_data);
    f.close();

    try repo.add("binary.bin");
    _ = try repo.commit("binary", "T", "t@t");

    // Get blob hash and read it back
    const tree = try execGit(allocator, tmp, &.{ "ls-tree", "HEAD", "binary.bin" });
    defer allocator.free(tree);
    var parts = std.mem.splitScalar(u8, trimRight(tree), ' ');
    _ = parts.next();
    _ = parts.next();
    const rest = parts.next() orelse return error.ParseError;
    var tab_parts = std.mem.splitScalar(u8, rest, '\t');
...
test "ziggit repo -> git fsck --no-dangling passes" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "c.txt", "ccc");
    try repo.add("c.txt");
...
test "ziggit commit parent chain -> git rev-list walks full history" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // git rev-list should count 5 commits
    const count_out = try execGit(allocator, tmp, &.{ "rev-list", "--count", "HEAD" });
    defer allocator.free(count_out);
    try std.testing.expectEqualStrings("5", trimRight(count_out));
    ...
    const head_out = try execGit(allocator, tmp, &.{ "rev-parse", "HEAD" });
    defer allocator.free(head_out);
    try std.testing.expectEqualStrings(&hashes[4], trimRight(head_out));
}

test "ziggit deeply nested 8 levels -> git ls-tree -r reads all" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    const deep_path = "a/b/c/d/e/f/g/h/deep.txt";
    try writeFileHelper(allocator, tmp, deep_path, "deep content");
    try repo.add(deep_path);
    _ = try repo.commit("deep", "T", "t@t");

    const tree = try execGit(allocator, tmp, &.{ "ls-tree", "-r", "HEAD" });
    defer allocator.free(tree);
    try std.testing.expect(std.mem.indexOf(u8, tree, "a/b/c/d/e/f/g/h/deep.txt") != null);

    // Verify content
    const content = try execGit(allocator, tmp, &.{ "show", "HEAD:a/b/c/d/e/f/g/h/deep.txt" });
    defer allocator.free(content);
    try std.testing.expectEqualStrings("deep content", content);
}

test "ziggit reopen repo -> state persists correctly" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // First: create repo, commit, tag
    {
        var repo = try ziggit.Repository.init(allocator, tmp);
        try writeFileHelper(allocator, tmp, "f.txt", "persisted");
        try repo.add("f.txt");
        _ = try repo.commit("persist test", "T", "t@t");
        try repo.createTag("v1.0.0", null);
        repo.close();
    }

    // Second: reopen and verify state
    {
...
test "ziggit commit -> git clone -> clone has correct HEAD and files" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    var repo = try ziggit.Repository.init(allocator, tmp);
    try writeFileHelper(allocator, tmp, "README.md", "# Hello\nThis is a test.\n");
    try writeFileHelper(allocator, tmp, "src/main.zig", "pub fn main() !void {}\n");
    try repo.add("README.md");
    try repo.add("src/main.zig");
    const hash = try repo.commit("initial commit", "Alice", "alice@example.com");
    try repo.createTag("v1.0.0", null);
    repo.close();
    ...
    // git clone
    _ = try execGit(allocator, "/root", &.{ "clone", tmp, clone_dir });

    // Verify HEAD matches
    const clone_head = try execGit(allocator, clone_dir, &.{ "rev-parse", "HEAD" });
    defer allocator.free(clone_head);
    try std.testing.expectEqualStrings(&hash, trimRight(clone_head));

    // Verify files exist
    const readme = try execGit(allocator, clone_dir, &.{ "show", "HEAD:README.md" });
    defer allocator.free(readme);
    try std.testing.expectEqualStrings("# Hello\nThis is a test.\n", readme);

    const main_zig = try execGit(allocator, clone_dir, &.{ "show", "HEAD:src/main.zig" });
    defer allocator.free(main_zig);
    try std.testing.expectEqualStrings("pub fn main() !void {}\n", main_zig);

    // Verify tag transferred
    const tags = try execGit(allocator, clone_dir, &.{ "tag", "-l" });
    defer allocator.free(tags);
    try std.testing.expect(std.mem.indexOf(u8, tags, "v1.0.0") != null);
}

test "ziggit commit -> git gc -> git fsck --strict passes" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Create several commits to give gc something to work with
    for (0..5) |i| {
        var buf: [32]u8 = undefined;
        const name = std.fmt.bufPrint(&buf, "file_{d}.txt", .{i}) catch unreachable;
        var content_buf: [32]u8 = undefined;
        const content = std.fmt.bufPrint(&content_buf, "content {d}", .{i}) catch unreachable;
        try writeFileHelper(allocator, tmp, name, content);
        try repo.add(name);
...
test "ziggit empty file -> git cat-file reports zero size" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "empty.txt", "");
    try repo.add("empty.txt");
    _ = try repo.commit("empty file commit", "T", "t@t");
...
test "ziggit commit -> git format-patch generates valid patch" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "app.js", "console.log('v1');\n");
    try repo.add("app.js");
    _ = try repo.commit("version 1", "Dev", "dev@co.com");

    try writeFileHelper(allocator, tmp, "app.js", "console.log('v2');\n");
    try repo.add("app.js");
    _ = try repo.commit("version 2", "Dev", "dev@co.com");

    const patch = try execGit(allocator, tmp, &.{ "format-patch", "-1", "HEAD", "--stdout" });
    defer allocator.free(patch);
    // Patch should contain commit message and diff
    try std.testing.expect(std.mem.indexOf(u8, patch, "version 2") != null);
    try std.testing.expect(std.mem.indexOf(u8, patch, "v2") != null);
}

test "ziggit commit -> git blame shows correct author" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "code.txt", "line1\nline2\nline3\n");
    try repo.add("code.txt");
    _ = try repo.commit("blamed", "BlameAuthor", "blame@test.com");

    const blame = try execGit(allocator, tmp, &.{ "blame", "code.txt" });
    defer allocator.free(blame);
    // Should have 3 lines
    var line_count: usize = 0;
    var iter = std.mem.splitScalar(u8, trimRight(blame), '\n');
    while (iter.next()) |_| line_count += 1;
    try std.testing.expectEqual(@as(usize, 3), line_count);
    // Should contain author
    try std.testing.expect(std.mem.indexOf(u8, blame, "BlameAuthor") != null);
}

test "ziggit commit -> git archive extracts with correct content" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "pkg/index.js", "module.exports = {};\n");
    try writeFileHelper(allocator, tmp, "pkg/package.json", "{\"name\":\"test\"}\n");
    try repo.add("pkg/index.js");
    try repo.add("pkg/package.json");
    _ = try repo.commit("packaged", "T", "t@t");

    // git archive to tar
    const tar_path = try std.fmt.allocPrint(allocator, "{s}/archive.tar", .{tmp});
    defer allocator.free(tar_path);
    _ = try execGit(allocator, tmp, &.{ "archive", "--format=tar", "-o", tar_path, "HEAD" });

    // Verify tar exists and has content
    const tar_file = try std.fs.openFileAbsolute(tar_path, .{});
    defer tar_file.close();
    const stat = try tar_file.stat();
    try std.testing.expect(stat.size > 0);
}

test "ziggit 3 tags at different commits -> git tag -l --sort=version:refname" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    try writeFileHelper(allocator, tmp, "f.txt", "v3");
    try repo.add("f.txt");
    _ = try repo.commit("v3", "T", "t@t");
    try repo.createTag("v3.0.0", null);

    const tags = try execGit(allocator, tmp, &.{ "tag", "-l", "--sort=version:refname" });
    defer allocator.free(tags);

    // All three tags should be listed
    try std.testing.expect(std.mem.indexOf(u8, tags, "v1.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, tags, "v2.0.0") != null);
    try std.testing.expect(std.mem.indexOf(u8, tags, "v3.0.0") != null);

    // git describe should point to v3.0.0
    const desc = try execGit(allocator, tmp, &.{ "describe", "--tags" });
    defer allocator.free(desc);
    try std.testing.expectEqualStrings("v3.0.0", trimRight(desc));
}

test "ziggit commit -> git cherry-pick into another branch works" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Use git to create a branch and cherry-pick
    {
        const o1 = try execGit(allocator, tmp, &.{ "config", "user.email", "t@t" });
        allocator.free(o1);
        const o2 = try execGit(allocator, tmp, &.{ "config", "user.name", "T" });
        allocator.free(o2);
        const o3 = try execGit(allocator, tmp, &.{ "checkout", "-b", "feature" });
        allocator.free(o3);
    }

    // Create a ziggit commit on feature branch
    {
        var repo2 = try ziggit.Repository.open(allocator, tmp);
        try writeFileHelper(allocator, tmp, "feature.txt", "feature work");
        try repo2.add("feature.txt");
        _ = try repo2.commit("feature commit", "T", "t@t");
        repo2.close();
    }
    ...
    {
        const o1 = try execGit(allocator, tmp, &.{ "checkout", "master" });
        allocator.free(o1);
        const o2 = try execGit(allocator, tmp, &.{ "cherry-pick", trimRight(feature_head) });
        allocator.free(o2);
    }

    // Verify file exists on master now
    const content = try execGit(allocator, tmp, &.{ "show", "HEAD:feature.txt" });
    defer allocator.free(content);
    try std.testing.expectEqualStrings("feature work", content);
}

test "ziggit large 4KB file -> git cat-file -p matches exactly" {
    const allocator = std.testing.allocator;
    const tmp = try makeTmpDir(allocator);
    defer {
        cleanupTmpDir(tmp);
    ...
    // Create a 4KB file with repeating pattern
    var large_content: [4096]u8 = undefined;
    for (0..4096) |i| {
        large_content[i] = @intCast(i % 128 + 32); // printable ASCII
    }

    const fp = try std.fmt.allocPrint(allocator, "{s}/large.txt", .{tmp});
    defer allocator.free(fp);
    {
        const f = try std.fs.createFileAbsolute(fp, .{ .truncate = true });
        defer f.close();
    ...
    const git_content = try execGit(allocator, tmp, &.{ "cat-file", "-p", "HEAD:large.txt" });
    defer allocator.free(git_content);
    try std.testing.expectEqual(@as(usize, 4096), git_content.len);
    try std.testing.expectEqualSlices(u8, &large_content, git_content);
}
